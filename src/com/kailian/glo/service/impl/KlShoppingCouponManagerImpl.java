...
import com.kailian.glo.service.IKlShoppingCouponManager;
import java.util.*;
...
@Service("klShoppingCouponManager")
@Transactional
public class KlShoppingCouponManagerImpl extends BaseManager<KlShoppingCoupon,java.lang.Long> implements IKlShoppingCouponManager{

	private IKlShoppingCouponDao klShoppingCouponDao;
	private ICmsDeleteBackupDao cmsDeleteBackupDao;

	/**增加setXXXX()方法,spring就可以通过autowire自动设置对象属性,请注意大小写*/
	public void setKlShoppingCouponDao(IKlShoppingCouponDao dao) {
		this.klShoppingCouponDao = dao;
	}
	...
	public EntityDao getEntityDao() {
		return (EntityDao)this.klShoppingCouponDao;
	}
	
	@Transactional(readOnly=true)
	public Page findPage(KlShoppingCouponQuery query) {
		return klShoppingCouponDao.findPage(query);
	}
	@Transactional(readOnly=true)
	...
	public void removeById(java.lang.Long id){
		KlShoppingCoupon klShoppingCoupon = this.getById(id);
		if (klShoppingCoupon != null) {
			Map map = new HashMap();
			map.put("object", klShoppingCoupon);
			String json = Utils.toJson(map);
			CmsDeleteBackup cmsDeleteBackup = new CmsDeleteBackup();
			cmsDeleteBackup.setTableName("KlShoppingCoupon");
...
public List<KlShoppingCoupon> queryTop(int count, String sort) {
	return this.klShoppingCouponDao.queryTop(count, sort);
}

public List<KlShoppingCoupon> queryTop(KlShoppingCouponQuery query) {
	return this.klShoppingCouponDao.queryTop(query);
}

public long queryCount(KlShoppingCouponQuery query) {
	return this.klShoppingCouponDao.queryCount(query);
}
