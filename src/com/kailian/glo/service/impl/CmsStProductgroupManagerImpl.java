...
import com.kailian.glo.service.ICmsStProductgroupManager;
import java.util.*;
...
@Service("cmsStProductgroupManager")
@Transactional
public class CmsStProductgroupManagerImpl extends BaseManager<CmsStProductgroup,java.lang.Long> implements ICmsStProductgroupManager{

	private ICmsStProductgroupDao cmsStProductgroupDao;
	private ICmsDeleteBackupDao cmsDeleteBackupDao;

	/**增加setXXXX()方法,spring就可以通过autowire自动设置对象属性,请注意大小写*/
	public void setCmsStProductgroupDao(ICmsStProductgroupDao dao) {
		this.cmsStProductgroupDao = dao;
	}
	...
	public EntityDao getEntityDao() {
		return (EntityDao)this.cmsStProductgroupDao;
	}
	
	@Transactional(readOnly=true)
	public Page findPage(CmsStProductgroupQuery query) {
		return cmsStProductgroupDao.findPage(query);
	}
	@Transactional(readOnly=true)
	...
	public void removeById(java.lang.Long id){
		CmsStProductgroup cmsStProductgroup = this.getById(id);
		if (cmsStProductgroup != null) {
			Map map = new HashMap();
			map.put("object", cmsStProductgroup);
			String json = Utils.toJson(map);
			CmsDeleteBackup cmsDeleteBackup = new CmsDeleteBackup();
			cmsDeleteBackup.setTableName("CmsStProductgroup");
...
public List<CmsStProductgroup> queryTop(int count, String sort) {
	return this.cmsStProductgroupDao.queryTop(count, sort);
}

public List<CmsStProductgroup> queryTop(CmsStProductgroupQuery query) {
	return this.cmsStProductgroupDao.queryTop(query);
}

public long queryCount(CmsStProductgroupQuery query) {
	return this.cmsStProductgroupDao.queryCount(query);
}
