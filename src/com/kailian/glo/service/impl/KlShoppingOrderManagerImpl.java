/*

 */

package com.kailian.glo.service.impl;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import com.kailian.glo.dao.ICmsDeleteBackupDao;
import com.kailian.glo.model.CmsDeleteBackup;
import com.kailian.glo.model.KlShoppingOrder;

import com.kailian.glo.model.KlShoppingOrder;
import com.kailian.glo.model.CmsStaticsFlow;
import com.kailian.glo.vo.query.KlShoppingOrderQuery;
import com.yebucuo.util.Utils;

import com.kailian.glo.service.IKlShoppingOrderManager;
import java.util.*;
import org.apache.commons.lang.StringUtils;

import com.yebucuo.common.base.*;
import com.yebucuo.common.util.*;

import cn.org.rapid_framework.util.*;
import cn.org.rapid_framework.web.util.*;
import cn.org.rapid_framework.page.*;
import cn.org.rapid_framework.page.impl.*;

import com.kailian.glo.model.*;
import com.kailian.glo.dao.*;
import com.kailian.glo.service.*;
import com.kailian.glo.vo.query.*;

import com.yebucuo.util.Utils;

/**
 * @author  wangdapeng
 * @version 1.0
 * @since 1.0
 */


@Service("klShoppingOrderManager")
@Transactional
public class KlShoppingOrderManagerImpl extends BaseManager<KlShoppingOrder,java.lang.Long> implements IKlShoppingOrderManager{

	private IKlShoppingOrderDao klShoppingOrderDao;
	private ICmsDeleteBackupDao cmsDeleteBackupDao;

	/**增加setXXXX()方法,spring就可以通过autowire自动设置对象属性,请注意大小写*/
	public void setKlShoppingOrderDao(IKlShoppingOrderDao dao) {
		this.klShoppingOrderDao = dao;
	}
	
	public void setCmsDeleteBackupDao(ICmsDeleteBackupDao dao) {
		this.cmsDeleteBackupDao = dao;
	}
	
	public EntityDao getEntityDao() {
		return (EntityDao)this.klShoppingOrderDao;
	}
	
	@Transactional(readOnly=true)
	public Page findPage(KlShoppingOrderQuery query) {
		return klShoppingOrderDao.findPage(query);
	}
	@Transactional(readOnly=true)
	public KlShoppingOrder getById(java.lang.Long id){
		return super.getById( id);
	}
	@Transactional
	public void removeById(java.lang.Long id){
		KlShoppingOrder klShoppingOrder = this.getById(id);
		if (klShoppingOrder != null) {
			Map map = new HashMap();
			map.put("object", klShoppingOrder);
			String json = Utils.toJson(map);
			CmsDeleteBackup cmsDeleteBackup = new CmsDeleteBackup();
			cmsDeleteBackup.setTableName("KlShoppingOrder");
			cmsDeleteBackup.setContent(json);
			cmsDeleteBackup.setCreatetimelong(System.currentTimeMillis());
			cmsDeleteBackup.setModifytimelong(0L);
			cmsDeleteBackup.setStatus(0);
			this.cmsDeleteBackupDao.save(cmsDeleteBackup);
		}
		
		super.removeById(id);
	}
	
	/** 插入数据 */
	@Transactional
	public void save(KlShoppingOrder entity){
		super.save(entity);
	}
	
	/** 更新数据 */
	@Transactional
	public void update(KlShoppingOrder entity){
		super.update(entity);
	}
	@Transactional(readOnly=true)
	public boolean isUnique(KlShoppingOrder entity, String uniquePropertyNames){
		return super.isUnique( entity, uniquePropertyNames);
	}
	@Transactional(readOnly=true)
	public List<KlShoppingOrder> findAll(){
		return super.findAll();
	}
	@Transactional
	public void saveOrUpdate(KlShoppingOrder entity) {
		super.saveOrUpdate(entity);
	}


public List<KlShoppingOrder> queryTop(int count, String sort) {
	return this.klShoppingOrderDao.queryTop(count, sort);
}

public List<KlShoppingOrder> queryTop(KlShoppingOrderQuery query) {
	return this.klShoppingOrderDao.queryTop(query);
}

public long queryCount(KlShoppingOrderQuery query) {
	return this.klShoppingOrderDao.queryCount(query);
}

public int updateCarProduct(long orderid, int status) {
	Map map = new HashMap();
	map.put("orderid", orderid);
	map.put("status", status);
	return this.klShoppingOrderDao.updateCarProduct("KlShoppingOrder.updateCarProduct", map);
}

public List<KlShoppingOrder> queryBySql(String sql) {
	return this.klShoppingOrderDao.queryBySql(sql);
}

}
