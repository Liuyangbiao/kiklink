...
<div class="reg">
     ...
     <div class="title">
            <ul>
              <li><input id="mobradio" type="radio" name="11" checked="checked" />手机注册</li>
              <!-- 
              <li onclick="window.location.href='${ctx}/kailian/CmsUserInfo/preregist2.do';"><input id="mailradio" type="radio" name="11"  />邮件注册</li>
               -->
            </ul>
     </div>
     <div class="form" >
         <form id="form1" action="${ctx}/kailian/CmsUserInfo/regist.do" method="post">
           ...
           <ul>
              <li><em>手机号码：</em>
                  <input id="mobile" name="mobile" value="${param.mobile}" type="text" class="input1" />
                  <!--<div class="lzm"><img src="${ctx}/image/nimg74.gif" /></div>-->
                  <div class="tips"><a href="#" id="bbbt" onclick="sendmobvalid('${ctx}',$('#mobile').val(),'bbbt');">发送验证码</a></div>
                   ${my:errordiv(map, 'mobile')}
              </li>
              <li><em>手机验证码：</em>
                  <input id="mobilevalidnum" name="mobilevalidnum" value="${param.mobilevalidnum}" type="text" class="input1" />
                  <div class="tips">输入收到的短信验证码</div>
                   ${my:errordiv(map, 'mobilevalidnum')}
              </li>
              <li><em>用户名：</em>
              ...
              <li><em>密码确认：</em>
                  ...
                  <div class="tips">验证密码一致性</div>
                   ${my:errordiv(map, 'repasswd')}
              </li>
              <li class="li_01">
                  <input type="checkbox" />成为我们的神秘访客
              ...
              <li>
              	<c:if test="${map.re le 0 and param.thirdType eq 'mobile'}">
     ...
     <!--
     	<form action="${ctx}/kailian/CmsUserInfo/regist.do" method="post">
           ...
           <ul>
             <li><em>电子邮箱：</em>
                 <input id="email" name="email" value="${param.email}" type="" class="input1" />
                 <div class="tips">用户手机号码和电子邮件都可以登录系统</div>
             </li>
             <li><em>密码：</em>
                 <input id="passwd" name="passwd" value="${param.passwd}" type="password" class="input1" />
             </li>
             ...
             <li>
               <input type="submit" class="btn1" value="注 册" />
               <input type="reset" class="btn1 btn2" value="重 置" />
            </li>
            <li>
...
$(document).ready(function(){
		...
		var paras = "mobile,passwd,repasswd";
		var pps = paras.split(",");
