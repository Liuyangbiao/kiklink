...
<title>焦点图${my:config('title后缀','--开练网')}</title>
...
<div class="fr w791">
      <div class="memberTitle"><h1>店铺管理 -  焦点图</h1></div>
              ...
              <tr>
                ...
                <c:choose>
                	<c:when test="${f.status gt 0}">
                		 <a href="${ctx}/kailian/KlSellerShop/focus.do?modid=${f.id}&tostatus=0">隐藏</a>
                	</c:when>
                	<c:otherwise>
                		 <a href="${ctx}/kailian/KlSellerShop/focus.do?modid=${f.id}&tostatus=1">显示</a>
                	</c:otherwise>
                </c:choose>
               
                <a href="${ctx}/kailian/KlSellerShop/focus.do?premodid=${f.id}#bianji">编辑</a>
                <a href="${ctx}/kailian/KlSellerShop/focus.do?delid=${f.id}">删除</a></td>
              </tr>
              </c:forEach>
              <!--
              <tr>
      ...
      <div class="focus_02">
      	<a name="bianji" ></a>
      	<form id="form1" action="${ctx}/kailian/KlSellerShop/focus.do" method="post">
      		<input id="focusid" name="focusid" type="hidden" value="${map.photo.id}" />
          <ul>
            <li><em>图片说明</em>
                <div class="input1"><input id="name" name="name" value="${map.photo.name}" type="text" /></div>
            </li>
            <li><em>图片</em>
                <div class="input1"><input id="resource" name="resource1" onclick="" value="${map.photo.resource}" type="text" /></div>
                 <input type="button" id="resource_button" class="btn2 fl" onclick="" value="上传照片..." />&nbsp;尺寸725*305
            </li>
            <li><em>链接地址</em>
                <div class="input1"><input id="comment" name="comment1" class="easyui-validatebox textbox" value="${map.photo.comment}" onblur="checkurl('comment')" type="text" data-options="validType:'kailianurl'">
                
                </div>输入你所要链接的页面，不可输入其他网站地址
            </li>
            <li><em>排序</em>
                <div class="input1"><input id="quanzhong" name="quanzhong" placeholder="默认10" class="easyui-numberbox" max=20 value="${map.photo.quanzhong}" type="text"/></div>
                数字越大，该图片就显示位置靠前，目前最大数字是20，默认是10
            </li>
...
function checkurl(tid) {
	var value = $('#' + tid).val();
	//alert(value);
	if (value == null || value.indexOf('http://')==0 && value.indexOf('http://www.kiklink.com') == 0 || value.indexOf('kiklink.com') >= 0)
	{
